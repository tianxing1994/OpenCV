...
即获得了, 亮度, 颜色, 方向特征.
将特征图进行高斯滤波到 4 个不同的尺度, 则亮度特征有 4 张图, 颜色空间有 2*4=8 张图, 方向特征有 4*4=16 张图.

步骤 2, 我们即可以通过不同尺度特征图之间的差分, 来获取高斯差分图 (DoG), 以体现中心外围差异性,
则亮度特征有 3 张中心外围差异图, 颜色空间有 6 张中心外围差异图, 方向特征有 12 张中心外围差异图.
也可以按下面 2.1.2 的马尔可夫方法来激活各个特征的显著图 (从不同尺度, 不同特征中获取显著度).

步骤 3, 将各特征的中心外围差异融合成一张显著图, 得到亮度, 亮度, 方向特征的显著图各一张.
将 3 种特征的 3 张显著图线性融合成一张, 作为最后的显著图. 
```
...
3. 如此迭代. 直到平衡时, 则得出了每一个像素的显著性.
4. 以由上 1-3 步得出的显著图像(激活图)体现的是每像素的显著性. 但是,
离显著像素更近的像素, 应该受到显著像素的加权. 因此, 还需要进行 "规范化"激活图(s3)
...
其中:
* A(p, q): 由 s2 得出的激活图中每个像素位置的值.
* F(i-p, j-q): 根据两像素之间的距离计算的权重. F(a, b) = exp(-(a^2+b^2) / (2σ^2))
```

```angular2html
批注:
"规范化"激活图(s3)
1. 由 s2 生成了激活图实际上是像素级别的, 每个像素的显著性. 考虑到实际情况中,
离显著像素更近的像素也应该具有显著性加成, 所以, 此 s3 步中,
每个像素根据自己与对方像素的距离及对方像素的显著值(激活值)来确定边权重.
2. 每个像素根据边权重向其它像素投票. 每一轮中, 每个像素拥有的票数是 s2 步中得出的激活值.
3. 通过不断地相互投票, 直到平衡时, 得出最后的显著图.
```
